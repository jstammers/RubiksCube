...
def main(_):
    with tf.Session() as sess:
        ...
        if GENERATE_DATA and not os.path.isfile("{0}Cube_Data.npz".format(N_CUBE)):
            state_list = []
            move_list = []
            ...
            np.savez("{0}Cube_Data.npz".format(N_CUBE),states=state_list,moves=move_list)
        else:
            npfiles = np.load("{0}Cube_Data.npz".format(N_CUBE))
